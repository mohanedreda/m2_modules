...
class session_category(models.Model):
    _name = 'custom.session'
    _description = 'Custom session'
    _rec_name = 'session'
    branding = fields.Char(string='Session')
...
class fashion_segment(models.Model):
...
class ProductTemplate(models.Model):
    ...
    product_specifications = fields.Many2one('product.specification', string='Product Specification')
    fashion_segments = fields.Many2one('product.segment', string="Fashion Segment")
    product_types = fields.Many2one('product.type', string='Product Type')
    checklist = fields.Boolean(string='Shoes')
    selection = fields.Selection(
        string='Select',
        selection=[('summer', 'Summer'), ('winter', 'Winter')]
    )
    category_id = fields.Many2one('custom.category', string='Custom Category')
    ...
    def _update_default_code(self):
        ...
        for record in self:
            ...
            brand = str(record.custom_category_id.code) if record.custom_category_id.code else ''
